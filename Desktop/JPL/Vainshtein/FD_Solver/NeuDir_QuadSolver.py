...
def Quad_disc(U_grid, Grid, Mult,  test = False, Ana = None):
    ...
    # We pad the grid to deal with boundary conditions
    # This handles Dirchlet automatically

    U = np.zeros((M1+2,M2+2))
    U[1:M1+1,1:M2+1] = U_grid[:M1,:M2]

    # Since finding test data that is compactly supported in a rectangle is hard
    # the approximator uses the function to get the actualy boundary data. 
    # If you don't do this then your approximation will always fail at the boundary for such functions.
    if test:
        ...
        # Enforces the Neumann condition
